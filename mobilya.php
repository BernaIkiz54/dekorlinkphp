...
if ($GelenMenuId != "") {
    $ToplamKayitSayisiSorgusu = $VeritabaniBaglantisi->prepare("SELECT *  FROM urun,kategori WHERE  urun.kategori_id=kategori.id AND kategori.ust_id=2 AND kategori.id=$GelenMenuId $AramaKosulu  ORDER BY kategori.id DESC");
    $ToplamKayitSayisiSorgusu->execute();
    $ToplamKayitSayisiSorgusu = $ToplamKayitSayisiSorgusu->rowCount();
    $SayfalamayaBaslanacakKayitSayisi = ($Sayfalama * $SayfaBasinaGosterilecekKayitSayisi) - $SayfaBasinaGosterilecekKayitSayisi;
    $BulunanSayfaSayisi = ceil($ToplamKayitSayisiSorgusu / $SayfaBasinaGosterilecekKayitSayisi);
} else {
    $ToplamKayitSayisiSorgusu = $VeritabaniBaglantisi->prepare("SELECT *  FROM urun,kategori WHERE  urun.kategori_id=kategori.id AND kategori.ust_id=2  $AramaKosulu  ORDER BY kategori.id DESC");
    $ToplamKayitSayisiSorgusu->execute();
    $ToplamKayitSayisiSorgusu = $ToplamKayitSayisiSorgusu->rowCount();
    $SayfalamayaBaslanacakKayitSayisi = ($Sayfalama * $SayfaBasinaGosterilecekKayitSayisi) - $SayfaBasinaGosterilecekKayitSayisi;
    $BulunanSayfaSayisi = ceil($ToplamKayitSayisiSorgusu / $SayfaBasinaGosterilecekKayitSayisi);
}
$AnaMenununTumUrunSayiSorgusu = $VeritabaniBaglantisi->prepare("SELECT Count(*) AS MenununToplamUrunu FROM urun,kategori WHERE urun.kategori_id=kategori.id AND kategori.ust_id=2");
                ...
                <tr>
                    <td>
                        <table width="250" align="center" border="0" cellpadding="0" cellspacing="0"
                            ...
                            <?php
                            $MenulerSorgusu = $VeritabaniBaglantisi->prepare("SELECT id,kategori_adi FROM kategori WHERE ust_id=2 ORDER BY kategori_adi ASC");
                        ...
                        <table width="795" align="center" border="0" cellpadding="0" cellspacing="0">
                            <tr><?php
                                if ($GelenMenuId != "") {
                                    ...
                                    $UrunlerSorgusu = $VeritabaniBaglantisi->prepare("SELECT *,urun.id as ID,kategori.kategori_adi as kategori FROM urun,kategori WHERE urun.kategori_id=kategori.id And kategori.ust_id=2 AND Durumu = '0'  $AramaKosulu ORDER BY kategori.id DESC LIMIT $SayfalamayaBaslanacakKayitSayisi, $SayfaBasinaGosterilecekKayitSayisi");
                                    $UrunlerSorgusu->execute();
                                    $UrunSayisi = $UrunlerSorgusu->rowCount();
                                    $UrunKayitlari = $UrunlerSorgusu->fetchAll(PDO::FETCH_ASSOC);
                                    $DonguSayisi = 1;
