...
if (($GelenEmailAdresi != "") and ($GelenSifre != "")) {
    $KontrolSorgusu = $VeritabaniBaglantisi->prepare("SELECT * FROM uyeler WHERE email = ? AND sifre= ? AND aktif_mi = ?");
    $KontrolSorgusu->execute([$GelenEmailAdresi, $MD5liSifre,0]);
    $KullaniciSayisi = $KontrolSorgusu->rowCount();
    $KullaniciKaydi = $KontrolSorgusu->fetch(PDO::FETCH_ASSOC);
    if($KullaniciSayisi>0){
        if($KullaniciKaydi["AktivasyonDurum"]==1){
            ...
            $MailIcerigiHazirla		=	"Merhaba Sayın " . $KullaniciKaydi["ad_soyad"] . "<br /><br />Sitemize yapmış olduğunuz üyelik kaydını tamamlamak için lütfen <a href='" . $SiteLinki . "/aktivasyon.php?AktivasyonKodu=" . $KullaniciKaydi["aktivasyon_anahtari"] . "&Email=" . $KullaniciKaydi["email"] . "'>BURAYA TIKLAYINIZ</a>.<br /><br />Saygılarımızla, iyi çalışmalar...<br />" . $SiteAdi;
            ...
            try{
                ...
                $MailGonder->addAddress(DonusumleriGeriDondur($KullaniciKaydi["email"]), DonusumleriGeriDondur($KullaniciKaydi["ad_soyad"]));
