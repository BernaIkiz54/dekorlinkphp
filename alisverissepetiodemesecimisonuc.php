...
if(isset($_SESSION["Kullanici"])){
	...
	if($GelenOdemeTuruSecimi!=""){
		if($GelenOdemeTuruSecimi=="Banka Havalesi"){
			$AlisverisSepetiSorgusu	=	$VeritabaniBaglantisi->prepare("SELECT * FROM sepetweb WHERE UyeId = ?");
			...
			if($SepetSayisi>0){
				foreach($SepetUrunleri as $SepetSatirlari){
					...
					$UrununToplamFiyati			=	($UrununFiyati	*$SepettekiUrunAdedi);
					...
					if($EklemeKontrol>0){
						$SepettenSilmeSorgusu	=	$VeritabaniBaglantisi->prepare("DELETE FROM sepetweb WHERE id = ? AND UyeId = ? LIMIT 1");
						$SepettenSilmeSorgusu->execute([$SepetIdsi, $SepettekiUyeId]);
						
						$UrunSatisiArttirmaSorgusu	=	$VeritabaniBaglantisi->prepare("UPDATE urun SET ToplamSatisSayisi=ToplamSatisSayisi + ? WHERE id = ?");
						$UrunSatisiArttirmaSorgusu->execute([$SepettekiUrunAdedi, $SepettekiUrunId]);	
						
						$StokGuncellemeSorgusu	=	$VeritabaniBaglantisi->prepare("UPDATE urun SET StokAdeti=StokAdeti - ? WHERE id = ? LIMIT 1");
