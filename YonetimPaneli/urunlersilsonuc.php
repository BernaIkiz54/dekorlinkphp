...
if(isset($_SESSION["Yonetici"])){
	...
	if($GelenID!=""){
		$UrunlerSorgusu			=	$VeritabaniBaglantisi->prepare("SELECT * FROM urun WHERE id = ?");
		...
		if($UrunlerSorgusuKontrol>0){
			...
			$UrunSilmeSorgusu	=	$VeritabaniBaglantisi->prepare("UPDATE urun SET Durumu = ? WHERE id = ? LIMIT 1");
			...
			if($UrunSilmeKontrol>0){
				...
				$FavorilerSilmeSorgusu	=	$VeritabaniBaglantisi->prepare("DELETE FROM favoriler WHERE urun_id = ?");
				$FavorilerSilmeSorgusu->execute([$GelenID]);

				$MenuGuncellemeSorgusu	=	$VeritabaniBaglantisi->prepare("UPDATE kategori SET UrunSayisi=UrunSayisi-1 WHERE id = ?");
