...
if(isset($_SESSION["Yonetici"])){
	...
	if($GelenID!=""){
		$UyeSilmeSorgusu	=	$VeritabaniBaglantisi->prepare("UPDATE uyeler SET aktif_mi = ? WHERE uyeID = ? LIMIT 1");
		$UyeSilmeSorgusu->execute([1, $GelenID]);
		$UyeSilmeKontrol	=	$UyeSilmeSorgusu->rowCount();

		if($UyeSilmeKontrol>0){
			$SepetSilmeSorgusu		=	$VeritabaniBaglantisi->prepare("DELETE FROM sepetweb WHERE UyeId = ?");
			...
			if($YorumlarSayisi>0){
				foreach($YorumlarKayitlari as $YorumSatirlari){
					...
					$UrunGuncellemeSorgusu	=	$VeritabaniBaglantisi->prepare("UPDATE urun SET YorumSayisi=YorumSayisi-1, ToplamYorumPuani=ToplamYorumPuani-? WHERE id = ? LIMIT 1");
